...
async function generateFullContent(analysisResult, synthesisData, registry, screenshotData) {
  ...
  // 2.5 Score Breakdown / How We Calculated (NEW POSITION - After exec summary)
  content += generateScoreBreakdownSection(analysisResult);
...
function generateExecutiveDashboard(analysisResult, synthesisData) {
  ...
  const { social_profiles, social_platforms_present } = analysisResult;
 ...
 */
function generateScoreBreakdownSection(analysisResult) {
  const { weight_reasoning, weights } = analysisResult;

  // Only show if AI grading was used
  ...
  html += '  <div style="display: grid; grid-template-columns: minmax(250px, 300px) 1fr; gap: 32px; align-items: center; margin-bottom: 12px;">\n';

  // Pie chart (SVG)
  html += '    <div style="position: relative;">\n';
  html += '      <svg viewBox="0 0 200 200" style="width: 100%; max-width: 300px; margin: 0 auto; display: block; transform: rotate(-90deg);">\n';
  ...
  html += '        <text x="100" y="95" text-anchor="middle" font-size="14" font-weight="600" fill="var(--text-primary)" transform="rotate(90 100 100)">Weight</text>\n';
  html += '        <text x="100" y="110" text-anchor="middle" font-size="13" font-weight="500" fill="var(--text-secondary)" transform="rotate(90 100 100)">Distribution</text>\n';
  html += '      </svg>\n';
  html += '    </div>\n';
...
function generateBenchmarkComparisonChart(analysisResult, synthesisData) {
  ...
  // Add notice if only partial data available
  if (categoriesWithStrengths.length > 0 && categoriesWithStrengths.length < strengthCategories.length) {
    html += '        <div style="background: var(--warning-lightest); padding: 16px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid var(--warning);">\n';
    html += `          <p style="margin: 0; font-size: 14px; opacity: 0.9;"><strong>📌 Note:</strong> Showing ${categoriesWithStrengths.length} of ${strengthCategories.length} strength categories with available data. ${matched_benchmark.company_name} excels in these areas based on current analysis.</p>\n`;
    html += '        </div>\n';
  }
...
function generateMultiPageScreenshotGallery(analysisResult, screenshotData, registry) {
  const { crawl_metadata, screenshot_desktop_url, screenshot_mobile_url } = analysisResult;
  const hasMultiPageData = crawl_metadata && crawl_metadata.pages && crawl_metadata.pages.length > 0;
  ...
  // If no multi-page data, show homepage screenshots with notice
  if (!hasMultiPageData) {
    ...
    // Show homepage screenshots from screenshotData
    if (screenshotData.screenshots && screenshotData.screenshots.length > 0) {
      ...
      // Desktop screenshot
      const desktopScreenshot = screenshotData.screenshots.find(s => s.device === 'desktop');
      if (desktopScreenshot && desktopScreenshot.dataUri) {
        html += '          <div>\n';
        ...
        html += `              <img src="${desktopScreenshot.dataUri}" alt="Homepage - Desktop" style="width: 100%; max-height: 400px; object-fit: cover; object-position: top; display: block;" />\n`;
        html += '            </div>\n';
        html += '          </div>\n';
      }

      // Mobile screenshot
      const mobileScreenshot = screenshotData.screenshots.find(s => s.device === 'mobile');
      if (mobileScreenshot && mobileScreenshot.dataUri) {
        html += '          <div>\n';
        ...
        html += `              <img src="${mobileScreenshot.dataUri}" alt="Homepage - Mobile" style="width: 100%; max-height: 500px; object-fit: cover; object-position: top; display: block;" />\n`;
        html += '            </div>\n';
        html += '          </div>\n';
      }
  ...
  // Original multi-page logic continues below...
  ...
  crawl_metadata.pages.forEach((page, idx) => {
    ...
    if (page.screenshot_paths.desktop) {
      ...
      html += '            <h4 style="font-size: 1rem; margin-bottom: 12px; opacity: 0.8;">🖥️ Desktop View</h4>\n';
      html += '            <div style="border: 2px solid var(--border-default); border-radius: 8px; overflow: hidden; background: var(--bg-secondary);">\n';
      html += `              <img src="${escapeHtml(desktopUrl)}" alt="Desktop screenshot" style="width: 100%; max-height: 400px; object-fit: cover; object-position: top; display: block;" loading="lazy" />\n`;
      html += '            </div>\n';
      html += '          </div>\n';
    }
    ...
    if (page.screenshot_paths.mobile) {
      ...
      html += '            <h4 style="font-size: 1rem; margin-bottom: 12px; opacity: 0.8;">📱 Mobile View</h4>\n';
      html += '            <div style="border: 2px solid var(--border-default); border-radius: 8px; overflow: hidden; background: var(--bg-secondary); max-width: 400px;">\n';
      html += `              <img src="${escapeHtml(mobileUrl)}" alt="Mobile screenshot" style="width: 100%; max-height: 500px; object-fit: cover; object-position: top; display: block;" loading="lazy" />\n`;
      html += '            </div>\n';
      html += '          </div>\n';
    }
