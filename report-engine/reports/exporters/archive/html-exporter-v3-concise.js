...
function generateExecutiveDashboard(analysisResult, synthesisData) {
  ...
  // Score breakdown moved to standalone section (now appears after Executive Summary)
...
function generateScoreBreakdownSection(analysisResult) {
  ...
  weightEntries.forEach(([dimension, weight]) => {
    ...
    html += `      <div style="background: var(--bg-tertiary); border-radius: 8px; height: 20px; position: relative; overflow: hidden; border: 1px solid var(--border-light);">\n`;
    html += `        <div style="background: var(--primary); height: 100%; width: ${weightPercent}%; border-radius: 6px; opacity: 0.9;"></div>\n`;
    html += '      </div>\n';
